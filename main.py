        ...
        while self.running:
            ...
            if self.state == "game":
                if self.event_handler.is_game_over:
                    self.session.on_session_end(survived=False, score=self.event_handler.score)
                    self.state = "gameover"
                    ...
                    self.session.on_session_end(survived=True, score=self.event_handler.score)
                    self.state = "win"
                    self.animatonic_controller.reset_animatonic()
                    self.office_controller.reset_office()
            ...
            if self.state == "win":
                ...
                if _prev_facing != self.event_handler._is_facing_office:
                    self.session.on_action("TurnLeft" if self.event_handler._is_facing_office else "TurnRight")
